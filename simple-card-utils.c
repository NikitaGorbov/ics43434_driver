...
int asoc_simple_startup(struct snd_pcm_substream *substream)
{
	...
	printk(KERN_INFO "MY in asoc_simple_startup\n");

	ret = asoc_simple_clk_enable(dai_props->cpu_dai);
	if (ret) {
		printk(KERN_INFO "MY in first if asoc_simple_clk_enable\n");
		return ret;
	}

	ret = asoc_simple_clk_enable(dai_props->codec_dai);
	if (ret){
		printk(KERN_INFO "MY in second if asoc_simple_clk_enable\n");
...
void asoc_simple_shutdown(struct snd_pcm_substream *substream)
{
	...
	struct simple_dai_props *dai_props =
		...
		printk(KERN_INFO "MY in asoc_simple_shutdown\n");

	if (dai_props->mclk_fs) {
		printk(KERN_INFO "MY in shutdown if dai_props->mclk_fs\n");
			  ...
			  struct snd_pcm_hw_params *params)
{
	...
	printk(KERN_INFO "MY hw_params\n");

	if (dai_props->mclk_fs) {
		printk(KERN_INFO "MY hw_params if dai_props->mclk_fs\n");
		mclk_fs = dai_props->mclk_fs;
	}

	if (mclk_fs) {
		printk(KERN_INFO "MY hw_params if mclk_fs\n");
	...
	printk(KERN_INFO "MY hw_params return 0\n");
	return 0;
err:
	return ret;
}
